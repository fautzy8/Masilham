                ...
                while let Some(msg) = message_receiver.recv().await {
                    match msg {
                        ServerMessage::StartMining(challenge, nonce_range, cutoff) => {
                            *last_activity_clone.lock().await = Instant::now();
                            println!("Chou , Shunpo: {} - {}", nonce_range.start, nonce_range.end);
                            ...
                            println!("Perfrct Jab: {} , Kun:{} , Way of:{:?} , Drahon:{:.3} k", best_difficulty,total,hash_time , ((total as f32)/hash_time.as_secs_f32())/1000.0 );
    ...
    match msg {
        ...
        Message::Binary(b) => {
            ...
            match message_type {
                    0 => {
                        if b.len() < 49 {
                            println!("Invalid data for Message Land of dawm");
                        } else {
